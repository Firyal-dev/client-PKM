"use client"

import { useState, useEffect, useMemo } from "react"
import { Calendar } from "@/components/ui/calendar"
import { Card, CardContent } from "@/components/ui/card"
import { PaginationControl } from "@/components/pagination-control"
import { EmptyState } from "@/components/ui/empty-user"
import { MapPin, CalendarDays, ArrowRight } from "lucide-react"
import { format, isSameDay } from "date-fns"
import { id } from "date-fns/locale"
import { PaginationProp } from "@/types/pagination-prop"
import type { Agenda } from "@/types/agenda-prop"

export default function Agenda({ data = [] }: { data: Agenda[] }) {
    const [date, setDate] = useState<Date | undefined>(new Date())
    const [currentPage, setCurrentPage] = useState(1)
    const itemsPerPage = 3

    // Filter data
    const filteredAgendas = useMemo(() => {
        return data.filter(item => 
            date ? isSameDay(new Date(item.date), date) : false
        )
    }, [data, date])

    // Reset page tiap ganti tanggal
    useEffect(() => {
        setCurrentPage(1)
    }, [date])

    // Pagination Logic
    const totalPages = Math.ceil(filteredAgendas.length / itemsPerPage)
    const paginatedAgendas = filteredAgendas.slice(
        (currentPage - 1) * itemsPerPage,
        currentPage * itemsPerPage
    )

    return (
        <section className="py-15" id="agenda">
            <div className="container mx-auto">
                <HeaderSection />

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto">
                    {/* Kalender */}
                    <aside className="lg:col-span-5 xl:col-span-4">
                        <CalendarCard 
                            date={date} 
                            onSelect={setDate} 
                            count={filteredAgendas.length} 
                        />
                    </aside>

                    {/* List Agenda */}
                    <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-3">
                        {paginatedAgendas.length > 0 ? (
                            <>
                                {paginatedAgendas.map((item) => (
                                    <AgendaItem key={item._id} item={item} />
                                ))}
                                
                                {paginatedAgendas.length > itemsPerPage && (
                                    <PaginationWrapper 
                                        currentPage={currentPage}
                                        totalPages={totalPages}
                                        onPageChange={setCurrentPage}
                                    />
                                )}
                            </>
                        ) : (
                            <EmptyState
                                title="Tidak Ada Kegiatan"
                                description="Tidak ada agenda kegiatan yang terjadwal pada tanggal ini."
                                icon={CalendarDays}
                            />
                        )}
                    </div>
                </div>
            </div>
        </section>
    )
}

// Header
function HeaderSection() {
    return (
        <div className="flex flex-col items-center text-center mb-12 space-y-3">
            <span className="px-4 py-1.5 rounded-full bg-blue-100 text-blue-600 text-xs font-semibold uppercase tracking-wide">
                Jadwal Kegiatan
            </span>
            <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-slate-900">
                Agenda Puskesmas
            </h2>
            <p className="max-w-2xl text-sm md:text-base text-slate-600 leading-relaxed">
                Cek jadwal kegiatan dan layanan kesehatan terkini di Puskesmas kami.
            </p>
        </div>
    )
}

// Calendar
function CalendarCard({ date, onSelect, count }: any) {
    return (
        <Card className="border-none shadow-md overflow-hidden rounded-2xl sticky top-24">
            <CardContent className="p-6 flex flex-col items-center">
                <Calendar
                    mode="single"
                    selected={date}
                    onSelect={onSelect}
                    locale={id}
                    className="rounded-md border-none w-full flex justify-center p-0"
                />
                <div className="mt-6 w-full pt-6 border-t border-slate-100">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Agenda Terpilih</p>
                    <div className="flex items-start justify-between gap-3">
                        <div className="flex flex-col">
                            <span className="text-base font-bold text-slate-900">
                                {date ? format(date, "d MMMM yyyy", { locale: id }) : "Belum dipilih"}
                            </span>
                            <span className="text-xs text-slate-500 mt-2">{count} Kegiatan Terjadwal</span>
                        </div>
                        <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-blue-50">
                            <CalendarDays className="w-5 h-5 text-blue-600" />
                        </div>
                    </div>
                </div>
            </CardContent>
        </Card>
    )
}

// Agenda
function AgendaItem({ item }: { item: Agenda }) {
    return (
        <Card className="group border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl bg-white overflow-hidden">
            <div className="flex">
                <div className="w-1.5 bg-blue-600 shrink-0" />
                <CardContent className="p-4 flex flex-col sm:flex-row gap-4 sm:items-center w-full">
                    <div className="min-w-[85px]">
                        <span className="text-base font-bold text-blue-600">{item.time}</span>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">{item.effective_date}</p>
                    </div>
                    <div className="hidden sm:block h-10 w-[1px] bg-slate-100 mx-2" />
                    <div className="flex-1 min-w-0">
                        <h4 className="text-base md:text-lg font-bold text-slate-900 group-hover:text-blue-600 truncate">
                            {item.activity_name}
                        </h4>
                        <div className="flex items-center gap-1.5 text-sm text-slate-500 mt-1">
                            <MapPin className="w-4 h-4 text-blue-400/60" />
                            <span className="truncate">{item.location}</span>
                        </div>
                    </div>
                    <div className="hidden sm:flex items-center justify-center w-10 h-10 rounded-full bg-slate-50 group-hover:bg-blue-50">
                        <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-blue-600" />
                    </div>
                </CardContent>
            </div>
        </Card>
    )
}

// Pagination
function PaginationWrapper({ currentPage, totalPages, onPageChange }: PaginationProp) {
    return (
        <div className="mt-10 py-4 border-t border-slate-100">
            <PaginationControl
                totalPages={totalPages}
                currentPage={currentPage}
                onPageChange={onPageChange}
            />
            <p className="text-center text-xs text-slate-500 mt-2">
                Menampilkan halaman {currentPage} dari {totalPages}
            </p>
        </div>
    )
}